...
class FixedNormalization(nn.Module):
    def __init__(self, mean, std):
        ...
        # Create a BatchNorm2d layer with 3 channels (for RGB images)
...
class DepthAnythingV2(nn.Module):
    def __init__(
        ...
        # Define fixed mean and std values for normalization
        mean = torch.tensor([[[[123.6750]], [[116.2800]], [[103.5300]]]])
        std = torch.tensor([[[[58.3950]], [[57.1200]], [[57.3750]]]])
        
        # Create the fixed normalization layer
        self.normalization = FixedNormalization(mean, std)
    ...
    def forward(self, x):
        ...
        # Create horizontally flipped version of the input
        ...
        # pred_mean = torch.mean(depth)
        # depth = depth + pred_mean
        # depth = 1.0 / depth
        depth = F.sigmoid(depth)
        
        # # Process flipped image
        features_flipped = self.pretrained.get_intermediate_layers(x_flipped, self.intermediate_layer_idx[self.encoder], return_class_token=True)
        depth_flipped = self.depth_head(features_flipped, patch_h, patch_w)
        depth_flipped = F.sigmoid(depth_flipped)
        
        # # Flip back the prediction of flipped image
        depth_flipped = torch.flip(depth_flipped, [3])
